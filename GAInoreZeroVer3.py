...
def individual(w, p_ran, p_uni):
    ...
    E_mc_now = E_mc
    E_now = E
    print E_mc_now, min(E_now)
    while E_now > 0 and E_mc_now > 0:
        r = random.random()
        if r <= p_ran:
            T, x = genRoundRandom(E_mc_now, E_now)
        elif p_ran < r <= p_ran + p_uni:
            T, x = genRoundUniform(E_mc_now, E_now)
        else:
            T, x = genRoundGreedy(E_mc_now, E_now, w)

        E_mc_now = E_mc_now + T * e_mc
        E_now = [E_now[j] - T * e[j] for j, _ in enumerate(node_pos)]
        print E_mc_now, min(E_now), max(x)
        row = [(u, xu) for u, xu in enumerate(x) if x[u] > 0]
        if not row:
            break
        temp = [0 for _ in charge_pos]
        for id, current in enumerate(row):
            u, xu = current
            if id == 0:
                time = time_move[-1][u]
            ...
            temp[u] = xu
            print E_mc_now, min(E_now)
            if E_mc_now < sum(E_move) or min(E_now) < 0:
                break
        indi["T"].append(T)
        indi["gen"].append(temp)
        if min(E_now) < 0:
            break
    print indi
    indi = injust(indi)
...
population = [individual(w, 0.3, 0.3) for _ in range(population_size)]
population = selectionBest(population)
print population[0]
indi = evolution(100, 0.8, 0.5, 0.3, 0.3, w)
